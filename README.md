...
Ajar is a proof of concept for an O(1) sized fetch client for APIs that implement the OpenAPI specification. It consumes a generic interface generated by [`openapi-typescript-codegen`](https://github.com/ferdikoomen/openapi-typescript-codegen) and returns a fully typed, deeply nested object that maps to fetch calls.
...
Currently, Ajar only implements a small subset of the OpenAPI specification[^2], however it should be more than enough for almost every project. Also it doesn't actually work right now because I did the hard part and didn't get around to the easy bit. Whoops.
